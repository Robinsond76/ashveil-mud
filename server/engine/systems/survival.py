...
async def survival_tick_handler(session, temp_label: str, is_sitting: bool = False) -> None:
    ...
    if session._state != State.COMBAT:
        sitting_stamina_tick(session.player, session.party, session.clock, is_sitting)
